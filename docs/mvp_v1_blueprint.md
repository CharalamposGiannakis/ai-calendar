...
The database now includes the import foundation: source documents, import batches, raw import rows, candidate events, and event-to-candidate provenance. The backend can safely store uploaded `.xlsx` files, create source-document and pending import-batch metadata, extract non-empty Excel rows into `import_rows`, generate pending reviewable `candidate_events`, and edit, reject, or transactionally approve candidates into real events. No frontend import-review UI exists yet.
...
1. Add a minimal frontend import-review UI.
2. Add PDF table import.
3. Add duplicate and conflict warnings.
