...
The database now includes the import foundation: source documents, import batches, raw import rows, candidate events, and event-to-candidate provenance. The application can safely store uploaded `.xlsx` files, create source-document and pending import-batch metadata, extract non-empty Excel rows into `import_rows`, generate pending reviewable `candidate_events`, and let the user edit, reject, or approve candidates into real events from a minimal browser UI.
...
2. Add duplicate and conflict warnings.
